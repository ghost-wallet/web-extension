...
export default function Sign({
  ...
  const fee = useMemo(() => {
    ...
    const inputValue = transaction.inputs.reduce(
      (acc: bigint, input: ITransactionInput) => {
    ...
    const outputValue = transaction.outputs.reduce(
      (acc: bigint, output: ITransactionOutput) => {
  ...
  const amount = useMemo(() => {
    ...
    const sentValue = transaction.outputs.reduce(
      (acc: bigint, output: ITransactionOutput) => {
        return acc + BigInt(output.value)
      },
      0n,
      ...
      Number(
        sentValue -
          (transaction.outputs.length === 1
            ? 0n
            : BigInt(
                transaction.outputs[transaction.outputs.length - 1].value,
              )),
      ) / 1e8
    )
  }, [transactions])
  ...
  const sign = useCallback(() => {
    ...
    request('account:sign', [transactions, password, inputs])
      .then((transactions) => {
