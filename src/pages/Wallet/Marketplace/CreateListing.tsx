...
const CreateListing: React.FC = () => {
  ...
  const { outputs, recipientError, amountError, handleAmountChange, handleMaxClick } = useTransactionInputs(
    token,
    maxAmount,
    kaspa.addresses[0],
  )
  ...
  return (
    <>
      ...
      <AnimatedMain className="flex flex-col h-screen w-full fixed">
        <Header title={`Create ${token.tick} Listing`} showBackButton={true} />
        <CryptoImage ticker={token.tick} size="large" />
        <div className="flex flex-col justify-between h-screen">
          <div className="flex flex-col items-center space-y-2 p-4">
            {/*Sell price amount goes here*/}
            <AmountInput
              value={outputs[0][1]}
